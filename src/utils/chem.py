...
def mol_from_frag(jbonds, frags=None, frag_smis=None):
    """
    params:
    jbonds: list of junction bonds in the molecule
    frags: list of blocks in rdkit.Mols format
    frag_smis: list of smiles strings for each block
    ...
    nfrags = len(frags)
    if nfrags == 0: return None, None
    
    # combine frags into a single molecule
    mol = frags[0]
    for i in np.arange(1,nfrags):
